...
class Test_kodning02(unittest.TestCase):
    def setUp(self):
        # Get the time
        ...
        # Get the filepath
        self.excel_src = os.path.join(shared_data_dir, 'kodning02.xlsx')

        # Instantiate the Excel class
        self.tcl = textblob_classifying.text(excel_src=self.excel_src, excel_dst=self.excel_dst)
        # Run it
        self.tcl.run_all()

    def test_wb(self):
        # Assert the sheetnames
        self.assertEqual(self.tcl.wb.sheetnames, ['Sheet'])
